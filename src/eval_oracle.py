...
class ModelEvaluator:
    ...
    def evaluate_df(self, df, k, aggregation_strategy, rerankers, output_folder_path):
        ...
        if output_folder_path:
            ...
            oracle_teIn_file_path = os.path.join(output_folder_path, "oracle_output.teIn")
            oracle_teOut_file_path = os.path.join(output_folder_path, "oracle_output.teOut")
        ...
        for _, row in tqdm(df.iterrows(), total=df.shape[0]):
            ...
            for reranker in rerankers:
                if isinstance(reranker, CodeReranker):
                    ...
                    # oracle reranker
                    # put the actual fids randomly in the top results
                    cur_len = len(search_results)

                    # first remove any fids that are already present in the search results
                    for o in search_results[:min(100, cur_len)]:
                        if o.fid in actual_fids:
                            o.fid = -1

                    # now randomly get len(actual_fids) indices from 0 to cur_len
                    indices = np.random.choice(min(100, cur_len), len(actual_fids), replace=False)
                    # print indices for this commit
                    print(f'Indices for commit {row["commit_id"]}: {indices}')
                    print(f'Actual fids for commit {row["commit_id"]}: {actual_fids}')
                    for idx, fid in zip(indices, actual_fids):
                        search_results[idx].fid = fid

                    # replace remaining -1 with random fids from later in the search results
                    random_fids = [o.fid for o in search_results[101:200] if o.fid not in actual_fids]
                    for idx, o in enumerate(search_results[:min(100, cur_len)]):
                        if o.fid == -1:
                            search_results[idx].fid = random_fids.pop(0)


                    # before reranking
                    oracle_dict[row['commit_id']] = search_results
            ...
            oracle_evaluation = self.eval_model.evaluate(oracle_dict[row['commit_id']], actual_fids)
            oracle_results.append(oracle_evaluation)
        ...
        if output_folder_path:
            ...
            generate_teIn_file(oracle_dict, oracle_teIn_file_path)
            evaluate_results(oracle_teIn_file_path, qry_file_path, oracle_teOut_file_path, self.eval_model)
        print('Before')
